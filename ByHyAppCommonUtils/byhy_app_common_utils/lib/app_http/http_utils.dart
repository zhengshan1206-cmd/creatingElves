...
import '../app_common/attribution/by_ascribe_util.dart';
import '../app_common/by_common_utils.dart';
import '../app_common/consts/log_utils.dart';
import '../app_ui/byhy_progress_hud.dart';
import 'apis.dart';
  ...
  static Future request<T>(
    ...
    DioUtils.instance.request(method, url,
      ...
      if (result is Map && result.containsKey('byuniplugin')) {
        ///归因插件事件
        ByAscribeUtil.byuniplugin(result['byuniplugin']);
      }
      if (showLoading == true) {
        EasyLoading.dismiss();
      }
      if (result['status'] == ExceptionHandle.success) {
        ...
        if (showMsgWhenFailed) {
          ByHyProgressHUD.showText(result['message']);
        }
        fail?.call(result['status'] ?? result["code"], result['message']);
      }
    }, onError: (code, msg) {
      if (showLoading == true) {
        ByHyProgressHUD.hide();
        EasyLoading.dismiss();
        ByHyProgressHUD.showError(msg);
      }
      fail?.call(code, msg);
    });
  }
