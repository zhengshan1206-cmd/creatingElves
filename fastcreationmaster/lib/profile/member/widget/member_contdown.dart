...
class MemberCountdown extends StatefulWidget {
  ...
  const MemberCountdown({
    super.key,
    this.fontSize = 12,
    this.textColor = ByColorUtil.colorF5,
    this.bgColor = ByColorUtil.color2E3038,
    this.separatorColor = ByColorUtil.colorF3,
...
class _MemberCountdownState extends State<MemberCountdown> {
  ...
  void getBeginTime() {
    if (widget.type != 0) {
      _beginTime = ByStorageUtils.getInt(Consts.kCancelPaySecondTime) ?? 0;
    } else {
  ...
  Duration _getRemain() {
    ...
    final timeDiff = _beginTime + Consts.kCancelPaySecondTimeDuration - now;
