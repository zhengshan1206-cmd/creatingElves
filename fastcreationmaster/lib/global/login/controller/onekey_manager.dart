 ...
 * @LastEditTime: 2026-02-25 13:43:56
 * @FilePath: /fastcreationmaster/lib/global/login/controller/onekey_manager.dart
 * @Description: 
 */
...
import 'package:fast_creation_master/core/service/debounce.dart';
...
class OneKeyManager {
  ...
  ///统计取号失败次数
  final List<String> failedList = ['first', 'second', 'third', 'fourth', 'fifth', 'more'];
  ...
  static void init({bool? checkLogin = true}) {
    ...
    if (checkLogin!) {
      ...
      if (user.userInfo != null &&
          user.userInfo?.isFormal != 0) {
        return;
      }
    }
    
    OneKeyLoginManager oneKeyLoginManager = OneKeyLoginManager();
    String appID = 'UZmlEXfd';
    if(Platform.isIOS) {
      appID = '9TVT0w0K';
    }
    oneKeyLoginManager.setTimeOutForPreLogin(6);
      ...
      if (code == 1000) {
        ...
        oneKeyLoginManager.getPhoneInfo().then((ShanYanResult shanYanResult) {
          ...
          if (shanYanResult.code == 1000) {
            ...
            DataService.onEvent('login_onekey_success', {'message': shanYanResult.message});
            print('________$shanYanResult');
          } else {
            DataService.onEvent('login_onekey_failed', {'message': shanYanResult.message, 'count': _instance.failedString});
            int index = _instance.failedList.indexOf(_instance.failedString) + 1;
  ...
  static void onekeyLogin({
    ...
    Function()? failed}) {
    if(Platform.isIOS) {
      DataService.onEvent('login', {'source': source, 'newer': Get.find<UserController>().userInfoBean.value!.activeDay! <= 1},);
    }
    else if(Platform.isAndroid) {
      DataService.onEvent('login', {'source': source,});
    }
    ...
    _instance.debouncer.run(() {
      ...
      OneKeyLoginManager oneKeyLoginManager = OneKeyLoginManager();
      if (Platform.isAndroid) {
        _instance.initAuthUI(oneKeyLoginManager, isBind: false, showClose: showClose!);
      } else if (Platform.isIOS) {
        _instance.initIOSAuthUI(oneKeyLoginManager, isBind: isBindMode!, showClose: showClose!);
      }
      ///拉起授权页
      oneKeyLoginManager.openLoginAuth().then((data) {
        ...
        if (1000 == data.code) {
          ...
          oneKeyLoginManager.setOneKeyLoginListener((data) {
            ...
            if (1000 == data.code) {
              DataService.onEvent('login_onekey', {'source': source});
  ...
  void authPullFailed(
    ...
    DataService.onEvent('login_phone', {'source': source});
    EventTracking.reportDataPoint(
                        pageTag: 'login_page',
                        operateType: 'view',
                        funcDetailImg: '',
                        funcDetailTag: '2',);
    Get.toNamed(Routes.loginPhone, arguments: {
      "isBind": isBindMode,
      "showClose": showClose,
  ...
  ///初始化闪验授权页UI
  void initAuthUI(OneKeyLoginManager oneKeyLoginManager, {bool isBind = false, bool showClose = true}) {
    ...
    shanYanUIConfig.androidPortrait.setLogoOffsetX = 24;
    ...
    shanYanUIConfig.androidPortrait.setAppPrivacyColor = ["#808080", "#98FC4A"];
    ...
    /// 是否勾选协议
    shanYanUIConfig.androidPortrait.setPrivacyState = !Get.find<UserController>().isAudit();
    shanYanUIConfig.androidPortrait.setCheckBoxHidden = !Get.find<UserController>().isAudit();
    ...
    ///手机号
    shanYanUIConfig.androidPortrait.setNumFieldOffsetY = 210;
    ...
    shanYanUIConfig.androidPortrait.setLogBtnTextColor = '#000000';
    shanYanUIConfig.androidPortrait.setLogBtnHeight = 48;
    shanYanUIConfig.androidPortrait.setLogBtnTextSize = 16;
    shanYanUIConfig.androidPortrait.setLogBtnBackgroundColor = '#98FC4A';
    ...
    const String appSloganID1 =
        "app_slogan1"; // 标识控件 id
    ShanYanCustomWidget appSlogan1 = ShanYanCustomWidget(
        appSloganID1, ShanYanCustomWidgetType.TextView);
    ...
    ShanYanCustomWidget appSlogan = ShanYanCustomWidget(
        "app_slogan", ShanYanCustomWidgetType.TextView);
    ...
    appSlogan.textAlignment = ShanYanCustomWidgetGravityType.left;
    shanyanCustomWidgetAndroid.add(appSlogan);

    ShanYanCustomWidget phoneLogin = ShanYanCustomWidget(
        "phone_login", ShanYanCustomWidgetType.Button);
    phoneLogin.textContent = "其他手机号登录";
    phoneLogin.top = !isBind ? 350 : 380;
    phoneLogin.left = 24;
    phoneLogin.right = 24;
    ...
    if (!isBind) {
      ///开启游客购买时
      if (Get.find<LaunchController>().launchInfo?.verConfig.allowTouristsVip ==
          1) {
        const String payString = "pay_btn"; // 标识控件 id
        ShanYanCustomWidget payBtn =
            ShanYanCustomWidget(payString, ShanYanCustomWidgetType.Button);
      ...
      ShanYanCustomWidget phoneBinding =
          ShanYanCustomWidget(phoneBindingString, ShanYanCustomWidgetType.Button);
  ...
  void initIOSAuthUI(OneKeyLoginManager oneKeyLoginManager, {bool isBind = false, bool showClose = true}) {
...
//供应商品牌标签("创蓝253提供认技术支持")
    ...
    shanYanUIConfig.ios.setCheckBoxHidden = !Get.find<UserController>().isAudit();
...
//    shanYanUIConfig.ios.layOutPortrait.setPrivacyHeight = 50;
    ...
    const String appSloganID =
        "app_slogan"; // 标识控件 id
    ShanYanCustomWidgetIOS appSlogan = ShanYanCustomWidgetIOS(
        appSloganID, ShanYanCustomWidgetType.TextView);
    ...
    // const String businessTipID =
    //     "business_tip"; // 标识控件 id
    // ShanYanCustomWidgetIOS businessTip = ShanYanCustomWidgetIOS(
    //     businessTipID, ShanYanCustomWidgetType.ImageView);
    // businessTip.backgroundImgPath = "icon_login_business_tip";
    // businessTip.top = 396;
    // businessTip.left = 28;
    // businessTip.height = 22;
    // businessTip.width = 132;
    // businessTip.isFinish = false;
    // businessTip.textContent = 'AAAAAAAAA';
    // businessTip.textColor = "#98FC4A";
    // businessTip.textFont = 24;
    // shanyanCustomWidgetIOS.add(businessTip);

    const String phoneLoginID = "phone_login"; // 标识控件 id
    ShanYanCustomWidgetIOS phoneLogin =
        ShanYanCustomWidgetIOS(phoneLoginID, ShanYanCustomWidgetType.Button);
    ...
    if (!isBind) {
      ...
      if (Get.find<LaunchController>().launchInfo?.verConfig.allowTouristsVip ==
          1) {
        const String payString = "pay_btn"; // 标识控件 id
        ShanYanCustomWidgetIOS payBtn =
            ShanYanCustomWidgetIOS(payString, ShanYanCustomWidgetType.Button);
        payBtn.textContent = "不登录直接购买";
        payBtn.top = 60;
        payBtn.height = 40;
        payBtn.right = 24;
      ...
      const String phoneBindingString = "binding_phone"; // 标识控件 id
      ShanYanCustomWidgetIOS phoneBinding =
          ShanYanCustomWidgetIOS(phoneBindingString, ShanYanCustomWidgetType.Button);
    ...
    ShanYanCustomWidgetIOS backBtn =
        ShanYanCustomWidgetIOS(backID, ShanYanCustomWidgetType.Button);
